...
const ContratsPage: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // Normalisation employer_nom, type_nom et fallback nature_contrat
      const normalized: Contrat[] = (c || []).map((x: any) => ({
        ...
        nature_contrat: x.nature_contrat || "emploi"
      }));
  ...
  const openEdit = (c: Contrat) => {
    ...
    setEditing({
      ...
      nature_contrat: c.nature_contrat || "emploi"
    });
    setFileToUpload(null);
    setIsModalOpen(true);
  };
  ...
  const validateBeforeSave = () => {
    ...
    if (editing.nature_contrat && editing.nature_contrat !== "emploi") {
      ...
      if (type?.duree_max_jours && editing.date_fin_contrat) {
        const diff = Math.ceil((new Date(editing.date_fin_contrat).getTime() - new Date(editing.date_debut_contrat).getTime()) / (1000*3600*24));
  ...
  const saveContrat = async () => {
    ...
    try {
      ...
      payloadObj["employer_id"] = typeof editing.employer === "object" ? (editing.employer as Employer).id : editing.employer;
      if (editing.type_contrat) payloadObj["type_contrat_id"] = typeof editing.type_contrat === "object" ? (editing.type_contrat as TypeContrat).id : editing.type_contrat;
            ...
            <TableBody>
              {filtered.map(c=>(
                <TableRow key={c.id}>
                  ...
                  <TableCell>{c.nature_contrat || "emploi"}</TableCell>
        ...
        <DialogContent aria-describedby="modal-description" className="sm:max-w-2xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            ...
            <div className="md:col-span-2">
              <Label>Description mission/prestation</Label>
