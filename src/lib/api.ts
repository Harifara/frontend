...
getStocksAutresMagasinsRaw: async (articleId: string) => {
  try {
    const token = await ensureKongToken();
    const res = await fetch(
      `${API_BASE_URL}/stock/stocks-autres-magasins/${cleanUUID(articleId)}`,
      { headers: getHeaders(token) }
    );

    if (res.status === 404) return []; // pas trouvé → tableau vide
    if (!res.ok) throw new Error("Impossible de récupérer le stock (Raw)");

    const text = await res.text();
    if (!text) return []; // réponse vide → tableau vide

    return JSON.parse(text); // JSON valide
  } catch (err) {
...
export const financeApi = {
  ...
  payerDepenseFinale: async (finalId: string): Promise<any> => {
    const token = await ensureKongToken();
    return fetchWithLog(`${API_BASE_URL}/finance/depenses-finales/${cleanUUID(finalId)}/payer/`, {
      method: "POST",
      headers: getHeaders(token),
    });
  },
